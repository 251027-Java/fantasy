...
public class RosterUserService {
    ...
    public Map<Long, String> getRosterUserIdToName(List<RosterUser> rosterUsers) {
        ...
        Set<String> userIds = rosterUsers.stream().map(RosterUser::getUserId).collect(Collectors.toSet());

        List<User> users = userRepo.findAllById(userIds);
        Map<String, String> userIdToDisplayName =
                users.stream().collect(Collectors.toMap(User::getUserId, User::getDisplayName));
