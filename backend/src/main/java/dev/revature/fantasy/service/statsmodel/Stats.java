...
public class Stats {
    ...
    public static MedianLuck computeMedianLuckScore(List<RosterUser> rosterUsers, List<List<WeekScore>> allScores) {
        ...
        for (int week = 0; week < allScores.size(); week++) {
            ...
            // Extract raw scores
            List<Double> scores =
                    weekScores.stream().map(WeekScore::getScore).sorted().toList();
     ...
     */
    public static Map<Long, AllPlayData> computeAllPlayLuckScore(
            List<RosterUser> rosterUsers, List<List<WeekScore>> allScores) {
        ...
        for (Long id : rosterUserIds) {
            ...
            double allPlayPct = (wins + losses + ties) == 0 ? 0 : (double) wins / (wins + losses + ties);
            ...
            double actualPct = (actualWins + actualLosses + actualTies) == 0
                    ? 0
                    : (double) actualWins / (actualLosses + actualWins + actualTies);

            double luckScore = (actualPct - allPlayPct) * 100;

            AllPlayData userAllPlayData =
                    new AllPlayData(allPlayPct, luckScore, wins, losses, ties, actualWins, actualLosses, actualTies);
